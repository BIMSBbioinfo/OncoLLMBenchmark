...
## define prompt to structure the clinical information of the PDAC cases into symptoms, medical history, diagnostic reports, pathology results and medications
...
1. Presenting Symptoms and Events: Extract details of the symptoms and events leading up to the patient’s current medical concern in a text (e.g. history of present illness, Oncology History, Review of System).
...
3. Diagnostic Reports: Extract any available diagnostic reports, including:
    ...
    * Physical exams: Extract relevant physical exam results.
...
#"Don't use bullet points, but rather seperate by comma.
...
## load data
df = pd.read_csv("/pdac_unstructured_plan_split.csv")
...
df.to_csv("pdac_structured.csv", index = False)
